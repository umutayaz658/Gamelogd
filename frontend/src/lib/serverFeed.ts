...
// over exactly as before — this must never throw and break the page render. Shared by both
// `/` (authenticated visitors) and `/home` (everyone, including anonymous) since both render
// the same HomeClient feed.
export async function fetchInitialFeed(): Promise<FeedItem[] | null> {
    try {
