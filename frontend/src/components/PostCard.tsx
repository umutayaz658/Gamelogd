...
export default function PostCard({ post, isDetailView = false }: PostCardProps) {
    return (
        <div className={`bg-zinc-900 border border-zinc-800 ${isDetailView ? 'rounded-none border-x-0 border-t-0' : 'rounded-2xl hover:bg-zinc-900/80'} p-4 transition-colors cursor-pointer`}>
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            <Link href={`/${post.user.username}`} className="font-bold text-white hover:underline">
                                {post.user.username}
                            </Link>
                            <Link href={`/${post.user.username}`} className="text-zinc-500 text-sm hover:text-zinc-400">
